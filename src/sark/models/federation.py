...
class FederationNodeBase(BaseModel):
    ...
    endpoint: str = Field(
        ..., description="Base URL for the node's API (e.g., https://sark.example.com)"
    )
    ...
    rate_limit_per_hour: int | None = Field(
        default=10000, description="Maximum requests per hour from this node"
    )
...
class ServiceDiscoveryRecord(BaseModel):
    ...
    discovered_at: datetime = Field(
        default_factory=datetime.utcnow, description="When this record was discovered"
    )
...
class DiscoveryQuery(BaseModel):
    ...
    method: DiscoveryMethod = Field(
        default=DiscoveryMethod.MDNS, description="Discovery method to use"
    )
...
class TrustEstablishmentRequest(BaseModel):
    ...
    challenge: str | None = Field(None, description="Challenge string for mutual verification")
...
class FederatedResourceRequest(BaseModel):
    ...
    capability_id: str | None = Field(None, description="Specific capability to invoke")
    principal_id: str = Field(..., description="Principal making the request")
    arguments: dict[str, Any] = Field(
        default_factory=dict, description="Arguments for capability invocation"
    )
...
class RouteResponse(BaseModel):
    ...
    recommended_route: RouteEntry | None = None
