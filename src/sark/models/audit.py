...
class AuditEvent(Base):
    ...
    principal_attributes = Column(JSONB, nullable=True)  # Structured principal data

    # Resource information
    resource_type = Column(String(50), nullable=True, index=True)  # "tool", "server", "policy"

    # Action details
    action_operation = Column(
        String(100), nullable=True, index=True
    )  # Specific operation performed
    action_parameters = Column(JSONB, nullable=True)  # Sanitized action parameters
